...
class ETFTrick:
    ...
    Attributes:
        ...
        _has_run (bool): Internal flag to check if the model has been fitted.
    """
    ...
    def __init__(self, future_specs: dict[str, dict[str, float]]):
        ...
        self._has_run = False
        self.K: pd.Series | None = None
        self.h: pd.DataFrame | None = None

    def run(
        self,
        etf_data: pd.DataFrame,
        rolling_dict: dict[str, NDArray],
        weights: NDArray[np.float64] | pd.DataFrame,
        value_one_point: NDArray | None = None,
    ) -> "ETFTrick":
        """
        ...
        Args:
            ...
            weights (NDArray | pd.DataFrame): Allocation weights. Can be a 1D array (fixed weights)
        ...
        K, h = self._core_engine_numba(
            K, h, phi, mat_weights, mat_open, mat_delta, sum_abs_weights, roll_mask
        )
    ...
    def _get_point_values(self, assets: list[str], value_one_point: NDArray | None) -> NDArray[np.float64]:
        ...
        if value_one_point is None:
            ...
            try:
                phi = np.array(
                    [self.future_specs[f"{asset}.c.0"]["point_value"] for asset in assets], dtype=np.float64
                )
            except KeyError as e:
                raise KeyError(f"Asset config not found in future_specs for key: {e}") from e
        else:
            phi = value_one_point.astype(np.float64)

        return phi

    def _get_price_matrices(
