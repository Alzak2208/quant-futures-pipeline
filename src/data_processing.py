...
from src.instrument_config import FUTURE_SPECS, BAR_SAMPLING_CONFIG
...
def data_cleaner_for_bars(ticks_dataset: pd.DataFrame) -> tuple[NDArray, pd.DataFrame]:
    ...
    df_clean = ticks_dataset.loc[
        ...
        ["instrument_id", "ts_event", "price", "size"],].copy()
...
def data_loader(data_dir: str, symbol: str, period_keyword: str) -> pd.DataFrame:
    ...
    if not found_files:
        raise FileNotFoundError(f"The file you are looking for has not been found")
    
    # Use the good file (should be only one and load it)
    file_path = found_files[0]
    df = pd.read_parquet(path=file_path,
                         columns=["instrument_id", "action", "ts_event", "price", "size"],
                         engine="pyarrow")
    
    
    # Put the good caracteristics of the symbol on the dataset loaded
...
def process_one_asset(asset: str,
                       datadir: str, 
                       month: str,
                       config: dict) -> Tuple[str, dict, pd.DataFrame]:
    """
...
def multi_assets_synchro_multi(assets: List[str],
                               datadir: str = "../data",
                               month: str = "Mar",
                               config: dict = BAR_SAMPLING_CONFIG) -> Tuple[Dict, pd.DataFrame]:
    """
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        # Submit tasks
        futures = {
            ex.submit(process_one_asset, asset, datadir, month, config): asset 
            for asset in assets
        }
